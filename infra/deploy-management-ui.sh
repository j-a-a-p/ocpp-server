...
# Simple Management UI Deployment Script
...
echo "Starting Management UI deployment..."
...
MANAGEMENT_UI_DEST="/var/www/html/management-ui"
...
DEPLOY_PACKAGE="management-ui-deploy-$(date +%Y%m%d-%H%M%S).tar.gz"
tar -czf "$DEPLOY_PACKAGE" -C management-ui/dist .
...
# Configuration
MANAGEMENT_UI_DEST="/home/ubuntu/management-ui"
BACKUP_DIR="/var/www/html/management-ui-backup"

# Clear existing files in the management-ui directory
echo "Clearing existing files..."
rm -rf "$MANAGEMENT_UI_DEST"/*

# Extract deployment package directly to the symbolic link location
echo "Extracting deployment package..."
tar -xzf management-ui-deploy.tar.gz -C "$MANAGEMENT_UI_DEST" --strip-components=0

# Set proper permissions
echo "Setting permissions..."
sudo chown -R www-data:www-data "$MANAGEMENT_UI_DEST"
sudo chmod -R 755 "$MANAGEMENT_UI_DEST"
...
mv management-ui-deploy-*.tar.gz management-ui-deploy.tar.gz
./deploy-remote.sh
rm -f management-ui-deploy.tar.gz deploy-remote.sh
SSH_COMMANDS

echo "Management UI deployment completed successfully!"
echo "Your application should be available at: http://apt.aircokopen.nu/management-ui"
echo "Backup of previous deployment is available at: /var/www/html/management-ui-backup" 